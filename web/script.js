...
// --- INTEGRACIÓN CON API FLASK (WEB REAL) ---
...
async function cargarDatosServidor() {
    try {
        ...
        const ventas = await apiFetch('/api/ventas');

        if (productos) baseDeDatos.productos = productos;
        if (ventas) baseDeDatos.ventas = ventas;
        if (usuarios) {
        ...
        console.log('Datos sincronizados desde la API de Flask');
        
        // Si tu script.js tiene una función para pintar/renderizar las tablas en pantalla, ejecútala aquí.
        // Ejemplo: si tienes un render() o inicializarVista(), llámalo aquí abajo.
    } catch (e) {
        console.error('Error inicializando datos web:', e);
    }
}

// Ejecutar la carga inmediatamente al cargar el documento
document.addEventListener('DOMContentLoaded', cargarDatosServidor);

// Objeto global dbSync adaptado para promesas web de Flask
window.dbSync = {
  getProductos: () => apiFetch('/api/productos'),
  getUsuarios: () => apiFetch('/api/usuarios'),
  getVentas: () => apiFetch('/api/ventas'),
  agregarProducto: (n, t, p, s, sm) => apiFetch('/api/productos', {
      method: 'POST',
      body: JSON.stringify({ nombre: n, tipo: t, precio: p, stock: s, stock_minimo: sm })
  }),
  registrarVenta: (cliente, ref, total, usuario, carrito) => apiFetch('/api/ventas', {
      method: 'POST',
      body: JSON.stringify({ cliente_nombre: cliente, referencia_pago: ref, total: total, usuario: usuario, carrito: carrito })
  })
};
